...
const CLIENT_REFRESH_URL = `${publicRuntimeConfig.SERVICE_PATH}/client/refresh`
...
export const loginSerivce = {
  login: (data: loginFormType) => {
    return new Promise<string>(async (resolve, reject) => {
      try {
        const result = await fetch(`${publicRuntimeConfig.SERVICE_PATH}/api/proxy${LOGIN_URL}`, {
          method: 'POST',
  ...
  silentRefresh: async () => {
    try {
      ...
      const result = await fetch(`${publicRuntimeConfig.SERVICE_PATH}/api/proxy${CLIENT_REFRESH_URL}`, {
        method: 'PUT',
      })
      if (result) {
        onSuccessLogin(await result.json())
      ...
      fetch(`${publicRuntimeConfig.SERVICE_API}/api/v1/token`)
          .then(res => {
